...
bool Fraction::test_divis(int divis)
{
	...
	if(((doubleNum / Ddivis) - std::floor(doubleNum / Ddivis)) == 0)
	{
		divNum = true;
	}

	if(((doubleDen / Ddivis) - std::floor(doubleDen / Ddivis)) == 0)
	{
		divDen = true;
	}
...
Fraction operator+(Fraction const & a, Fraction const & b)
{
	return Fraction (
		...
		a.get_denominateur() * b.get_denominateur()
		);
}
...
Fraction operator/(Fraction const & a, Fraction const & b)
{
	return Fraction(
		...
		a.get_denominateur() * b.get_numerateur()
	);
}
